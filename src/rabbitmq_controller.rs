...
async fn main() -> Result<()> {
    ...
    if cmd == String::from("export") {
        ...
        run_controller::<deps_hack::RabbitmqCluster, RabbitmqCluster, RabbitmqReconciler, RabbitmqReconcileState>(false).await?;
        println!("controller terminated");
    } else if cmd == String::from("crash") {
        println!("running rabbitmq-controller in crash-testing mode");
        run_controller::<deps_hack::RabbitmqCluster, RabbitmqCluster, RabbitmqReconciler, RabbitmqReconcileState>(true).await?;
        println!("controller terminated");
    } else {
